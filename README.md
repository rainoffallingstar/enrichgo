...
- `--idmaps-level extended`：使用 NCBI `gene_info` 官方映射回填 `symbol -> entrez`（覆盖比 basic 更全，但仍更慢）
...
- 通用：
  ...
  - `--update-db-idmaps` / `--update-db-idmaps-level` 控制更新时 ID 映射刷新（默认会刷新，`--update-db-idmaps=true`；`extended` 当前离线只回填 `symbol -> entrez`）
